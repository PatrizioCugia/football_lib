...
def project_data(df, columns_to_project):
    ...
    principal_components = pca.fit_transform(df[columns_to_project])
...
def find_similar_players(df_pca, player_name, num_similar=10):
    ...
    player_projection = df_pca[df_pca['Name'] == player_name][['PC1', 'PC2']].values[0]
    df_pca['distance'] = df_pca.apply(lambda row: distance.euclidean(player_projection, [row['PC1'], row['PC2']]), axis=1)
    similar_players_df = df_pca.sort_values(by='distance').head(num_similar + 1)
...
def print_important_features(pca, columns, num_features=5):
    for i in range(2):
        ...
        important_features = [(columns[idx], component[idx]) for idx in indices[:num_features]]
